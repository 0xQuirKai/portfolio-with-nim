...
- Verifying conditions like user age eligibility (example: age at least 18)
...
The prover generates a proof π using the function Prove(x, r) and submits it to the blockchain.

The smart contract only performs verification of the proof π and returns true or false, without accessing x.
...
where each state si represents a step in the computation.

The proof guarantees that there exists a valid execution trace T such that F(T) is a valid execution, without revealing T.
...
- ZK proof is created using SNARK/STARK logic depending on the case  
