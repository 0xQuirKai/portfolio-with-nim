...
This introduces the constraint: "x ≥ 18" without revealing the actual value of x.
...
The constraint is defined as: (x - 18) * r = 0

This ensures that only valid inputs can generate a valid proof.

The prover generates a proof π = Prove(x, r) and submits it to the blockchain.

The smart contract only performs Verify(π) = true, without ever accessing x.
...
Instead of proving a single constraint, the system proves correctness of an entire computation trace T = {s0, s1, ..., sn}, where each si represents a state transition.

The proof guarantees that there exists a valid execution trace T such that F(T) = valid execution, without revealing T.
